...
function cargarBadgesSidebar(negocioId = null) {
  ...
  try {
    let qPend = db.collection('reservas').where('estado', '==', 'pendiente');
  ...
  return new Promise((resolve) => {
    auth.onAuthStateChanged(async (user) => {
      ...
      if (rolRequerido === 'gerente') {
        ...
        window.LINK_PUBLICO_NEGOCIO = `${_base}/public/reserva.html?negocio=${encodeURIComponent(negocio.slug || negocio.id || '')}`;
